...
import { Plus, Search, User, Phone, Mail, Car } from 'lucide-react';
import { orders } from '@/data/mockData';

// Extract unique clients and count their vehicles
const clientsMap = new Map<string, { client: typeof orders[0]['client']; vehicleCount: number }>();
orders.forEach(order => {
  const existing = clientsMap.get(order.client.id);
  if (existing) {
    existing.vehicleCount += 1;
  } else {
    clientsMap.set(order.client.id, { client: order.client, vehicleCount: 1 });
  }
});
