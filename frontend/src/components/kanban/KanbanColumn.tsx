          ...
          >
            <CardContent className="p-4">
              ...
              <p className="text-xs text-muted-foreground mb-3">
                {order.vehicle.plate} • {order.vehicle.year} • {order.vehicle.color}
              </p>
                ...
                {order.services.slice(0, 2).map((service) => (
                  <div key={service.id} className="flex items-center gap-2">
                    <div className={cn(
                      ...
                      service.status === 'completed' ? 'bg-status-ready' :
                      service.status === 'in_progress' ? 'bg-status-progress' : 'bg-muted-foreground'
                    )} />
